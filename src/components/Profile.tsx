    ...
    if (!userData.phone || userData.phone.trim() === '') {
      if (phoneInput && phoneInput !== "+91 " && phoneInput.trim() !== '') {
        ...
        try {
          ...
          if (!phoneDocs.empty) {
            setMsg({ text: 'Phone number already used by another account.', type: 'error' });
            setLoading(false);
            return;
          }
        } catch (e: any) {
        ...
        <div className="w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input name="name" type="text" defaultValue={userData.name} className="w-full bg-[#0A0B0D] border border-transparent rounded-2xl py-4 pl-14 pr-5 text-white focus:border-blue-500/50 focus:outline-none transition-colors shadow-inner font-medium text-[15px]" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" value={user} disabled className="w-full bg-[#0A0B0D] border border-transparent rounded-2xl py-4 pl-14 pr-5 text-white opacity-70 cursor-not-allowed text-[15px]" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        readOnly={!!userData.phone && userData.phone.trim() !== ''}
                        disabled={!!userData.phone && userData.phone.trim() !== ''}
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-2 font-medium">Account Number</label>
                    <div className="relative">
                      <input type="text" value={userData.accountNumber || '8845 2210 9942'} disabled className="w-full bg-[#0A0B0D] border border-transparent rounded-2xl py-4 px-6 text-white opacity-70 cursor-not-allowed tracking-wider text-[15px]" />
                    </div>
                  </div>
                </div>

                {msg.text && (
                  <div className={`p-4 rounded-xl text-sm justify-center flex mt-6 ${msg.type === 'error' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}>
                    {msg.text}
                  </div>
                )}

                <div className="pt-2">
                  <button disabled={loading} type="submit" className="w-full bg-[#1b64f2] hover:bg-blue-600 text-white font-medium py-4 rounded-2xl transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-[15px]">
          ...
          {activeTab === 'security' && (
            <div className="bg-[#16191F] border border-white/5 rounded-3xl p-8 shadow-xl">
              <h3 className="text-xl font-semibold mb-4">Two-Factor Authentication (MFA)</h3>
              ...
              {userData.twoFactorEnabled ? (
                <div className="flex flex-col gap-8 border border-white/5 bg-[#1C2028] p-8 md:p-10 rounded-3xl relative">
                  ...
                  <div className="flex-1 w-full space-y-6 relative z-10">
                    <div>
                      <h4 className="text-xl xl:text-2xl text-white font-medium mb-2 border-b border-white/10 pb-3">Security & Auth Policies</h4>
                    </div>
                    
                    <div className="space-y-4">
                      <div className="flex items-center justify-between bg-black/40 p-5 rounded-2xl border border-white/5 shadow-inner">
                        <div>
                          <h4 className="text-white font-medium">Require 2FA for Login</h4>
                          <p className="text-sm text-gray-500 mt-1">Ask for authenticator code when signing in</p>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input 
                            ...
                            onChange={async () => await updateUserProfile(user, { require2FAForLogin: userData.require2FAForLogin === false ? true : false })}
                          />
                      ...
                      <div className="flex items-center justify-between bg-black/40 p-5 rounded-2xl border border-white/5 shadow-inner">
                        <div>
                          ...
                          <p className="text-sm text-gray-500 mt-1">Ask for code to verify Transfer, Deposit, and Bill Pay</p>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input 
                            type="checkbox" 
                            className="sr-only peer"
                            checked={userData.require2FAForTransactions === true}
                            onChange={async () => await updateUserProfile(user, { require2FAForTransactions: userData.require2FAForTransactions === true ? false : true })}
                          />
                          <div className="w-11 h-6 bg-gray-800 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-500 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#B88A44]"></div>
                        </label>
                      </div>
                    ...
                    <div className="pt-4 flex justify-end">
                      ...
                      >
                        {mfaLoading ? 'Processing...' : 'Disable 2FA / Remove Protection'}
                      </button>
                    </div>
                  </div>
                </div>
                  ...
                  {!mfaData ? (
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-center">
                      ...
                      <div className="flex-1 space-y-5 w-full">
                        <div>
                          <h4 className="font-semibold text-lg mb-1">Scan this QR Code</h4>
                          <p className="text-sm text-gray-500">Open Google Authenticator and scan the code to link your account.</p>
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-gray-500 uppercase mb-2">Enter 6-digit verification code</label>
                          <input 
                            type="text" 
                            value={mfaToken}
              ...
              {mfaMsg.text && (
                <div className={`mt-6 p-4 rounded-xl text-sm justify-center flex ${mfaMsg.type === 'error' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}>
                  {mfaMsg.text}
                </div>
              )}
            </div>
            ...
            <div className="bg-[#16191F] border border-white/5 rounded-3xl p-8 shadow-xl">
              ...
              {!userData.notifications || userData.notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center min-h-[200px] text-center">
                  ...
                  <p className="text-sm text-gray-500 mt-2">You have no new notifications.</p>
                </div>
              ) : (
          ...
          {activeTab === 'password' && (
            <div className="bg-[#16191F] border border-white/5 rounded-3xl p-8 shadow-xl">
              <form onSubmit={handlePasswordChange} className="space-y-6 max-w-md">
                <div>
                  <label className="block text-sm text-gray-400 mb-3">Old Password</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-3">New Password</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <Key className="w-5 h-5 text-gray-500" />
                    </div>
                ...
                {pwdMsg.text && (
                  <div className={`p-4 rounded-xl text-sm justify-center flex mt-6 ${pwdMsg.type === 'error' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}>
                    {pwdMsg.text}
                  </div>
                )}

                <div className="pt-4">
                  <button disabled={pwdLoading} type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-4 rounded-2xl transition-all disabled:opacity-50">
